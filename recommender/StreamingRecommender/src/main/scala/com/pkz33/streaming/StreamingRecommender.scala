...
object ConnHelper extends Serializable {
  lazy val jedis = new Jedis("pkz33")
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://pkz33:27017/recommender"))
}
...
object StreamingRecommender {
      ...
      rdds =>
        rdds.foreach {
          case (uid, mid, score, timestamp) => {
            ...
            // 2. 从相似度矩阵中取出与当前电影最相似的N个电影，作为备选列表，Array[mid]
  ...
  def getUserRecentlyRatings(num: Int, uid: Int, jedis: Jedis): Array[(Int, Double)] = {
    // 从redis读取数据，用户评分数据保存在uid:UID为key的列表中，value是MID:SCORE
  ...
  : Array[(Int, Double)] = {
    ...
    // 根据备选电影的mid做groupby，根据公式求得最后的推荐评分
