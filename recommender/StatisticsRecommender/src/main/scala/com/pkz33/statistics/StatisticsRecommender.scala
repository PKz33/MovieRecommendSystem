...
object StatisticsRecommender {
  ...
  // 统计表的名称
  val HOT_MOVIES = "HotMovies"
  val RECENTLY_HOT_MOVIES = "RecentlyHotMovies"
  val MOVIES_AVERAGE_SCORE = "MoviesAverageScore"
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 3. 优质电影统计，统计电影的平均评分
    val moviesAverageScoreDF = spark.sql("select mid, avg(score) as avg from ratings group by mid")
    storeDFInMongoDB(moviesAverageScoreDF, MOVIES_AVERAGE_SCORE)
    ...
    // 把平均评分加入到movie表里，加一列
    val movieWithScore = movieDF.join(moviesAverageScoreDF, "mid")
    ...
    val genresTopMoviesDF = genresRDD.cartesian(movieWithScore.rdd)
      .filter {
        // 条件过滤，找出movie的字段genres值（如Action|Adventure|Sci-Fi）包含当前类别genre（如Action）的
